...
// оно сука не запускается, ну похуй с ним, главное, что по ссылочкам можно тыкать.
...
class StarWarsFilm(id: EntityID<Int>) : Entity<Int>(id) {
    ...
    var actors   by Actor via StarWarsFilmActors // SizedIterator - ? ссылка на таблицу, используя primaryKey
    val ratings  by UserRating referrersOn UserRatings.film // SizedIterator - ? ссылка на таблицу, используя колонку с reference
}
